    ...
    $.fn.split = function(options) {
        ...
        if (spliters.length == 0) { // first time bind events to document
            $(document.documentElement).bind('mousedown.spliter', function() {
				...
				//alert('custom-scroll');
                var h = $(".leftTopSplitterSpacing").height() - $(".rightInnerBlocks").height() - $(".canvasBlockInnerBlock").height() - $(".regionNameSearch").height() 
                h=h-10;
                $('.leftSliderScrollRegion .ScrollPaneBlock').height(h);
